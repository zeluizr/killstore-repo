...
function quitarDecimalesCart() {
  if (location.hash == '#/cart') {
    setTimeout(() => {
      ...
      if (
        document.querySelector('tbody.totalizers-list td.monetary') != null &&
        document.querySelector('tbody.totalizers-list td.monetary').innerText.includes(',')
      ) {
        quitarDecimales(document.querySelector('tbody.totalizers-list td.monetary'))
      }
      if (
        document.querySelector('tr.srp-summary-result td.monetary') != null &&
        document.querySelector('tr.srp-summary-result td.monetary').innerText.includes(',')
      ) {
        quitarDecimales(document.querySelector('tr.srp-summary-result td.monetary'))
      }
      if (
        document.querySelector('.summary-template-holder tfoot tr td.monetary') != null &&
        document.querySelector('.summary-template-holder tfoot tr td.monetary').innerText.includes(',')
      ) {
        quitarDecimales(document.querySelector('.summary-template-holder tfoot tr td.monetary'))
      }
      if (
        $('.summary-template-holder .summary-coupon p.coupon-fields span input').attr('placeholder') !=
        'Código cupón de descuento'
      ) {
        $('.summary-template-holder .summary-coupon p.coupon-fields span input').attr(
          'placeholder',
          'Código cupón de descuento'
        )
      }
      if ($('a.boton-volver-home').length == 0 && $('.table.cart-items tbody tr').length >= 1) {
        $('.transactions-container.row-fluid').append(
          "<a class='boton-volver-home' href='/'><p>←</p> Seguir comprando</a>"
        )
      }
    }, 1000)
  }
  ...
  if (location.hash == '#/payment') {
    setTimeout(() => {
      ...
      if (
        document.querySelector('.shp-summary-group-price.vtex-omnishipping-1-x-SummaryItemPrice') != null &&
        document
          .querySelector('.shp-summary-group-price.vtex-omnishipping-1-x-SummaryItemPrice')
          .innerText.includes(',')
      ) {
      ...
      //Precio Webpay
      if (
        document.querySelector('fieldset.webpayPaymentGroup .installments p.sight span') != null &&
        document.querySelector('fieldset.webpayPaymentGroup .installments p.sight span').innerText.includes(',')
      ) {
        quitarDecimales(document.querySelector('fieldset.webpayPaymentGroup .installments p.sight span'))
      }
      //Precio Transferencia
      if (
        document.querySelector('fieldset.promissoryPaymentGroup .installments p.sight span') != null &&
        document.querySelector('fieldset.promissoryPaymentGroup .installments p.sight span').innerText.includes(',')
      ) {
...
!(function (e) {
  ...
  function (e, o) {
    e.exports = class {
      constructor({ dbg: e = !1, logo: o = '<img src="https://via.placeholder.com/160x60">' } = {}) {
    ...
    e.exports = class {
              ...
              (a.priceTags.length
                ? a.priceTags.filter((e) =>
                    e.ratesAndBenefitsIdentifier
                      ? e.ratesAndBenefitsIdentifier.matchedParameters['couponCode@Marketing'] == o
                      : 0
                  ).length
                : 0)
            )
            ...
            $.each(e.items, function (e) {
              this.priceTags.length > 0 &&
                ...
                $(`.table.cart-items tbody tr.product-item:eq(${e})`)
                  ...
                  .find('.product-name')
                  .append(`<span class="v-custom-addLabels-active-flag">${o}</span>`))
            })
        } catch (e) {
          console.error(e)
        }
            ...
            let s = e.items.reduce(
              (e, o) => (
                null != o.parentItemIndex &&
                  ((e[o.parentItemIndex] = e[o.parentItemIndex] || []), e[o.parentItemIndex].push(o)),
                e
              ),
              {}
            )
                ...
                for (var r in n) {
                  ...
                  $(`.mini-cart .cart-items > li:eq(${a}) > .v-custom-bundles`).append(
                    `\n                <div class="hproduct item v-custom-indexed-item" data-sku="${
                      ...
                      o.name
                    }" href="${o.detailUrl}">${o.name}</span>\n                  <span class="quantity badge">${
                      o.quantity
                    }</span>\t\t\t\t\n                  <div class="description">\n                    <strong class="price pull-right" data-bind="text: sellingPriceLabel">${
                      e.storePreferencesData.currencySymbol
                    } ${t(e.clientPreferencesData.local, e.storePreferencesData.currencyCode, o.sellingPrice).toFixed(
                      2
                    )}</strong>\n                  </div>\n                </div>\n              `
                  ),
            ...
            if ('' != n && n.match(/(day)|(dia)|(día)/gm)) {
              ...
              if (a) {
                ...
                $(this)
                  ...
                  .text()
                  .toLowerCase()
                  .match(/(ready in up)|(pronto)|(a partir de)|(hasta)/gm) && (n = e.lang.PickupDateText),
                  $(this)
                    .find(o.join(', '))
            ...
            $.each(a, function (o) {
              ...
              if ('' != r && r.match(/(day)|(dia)|(día)/gm)) {
                ...
                if (o) {
                  ...
                  r.toLowerCase().match(/(ready in up)|(pronto)|(A partir de)|(hasta)/gm) &&
              ...
              r
                .hide()
                .after(
                  `<p class="black-50 mt3 mb0 js-shippingPreviewPackges">${r.text().split(':')[0]}: ${n.join('; ')}</p>`
                )
      ...
      condensedTaxes(e) {
        ...
        let a = `\n      <div class="vcustom-customTax-resume">\n       ${o
            .map(
              (o) =>
                `<p class="vcustom-customTax-resume__i"><span class="n">${o.name}</span><span class="v">${
                  e.storePreferencesData.currencySymbol
        ...
        n.length &&
          ...
          n.find('.info').append(`<div class="vcustom-customTax-tot"><span>?</span> ${a}</div>`))
      }
      update(e) {
        let o = this
      ...
      updateLang(e) {
        ...
        if (
          ...
          o.identifiedUserMessage &&
            $('.identified-user-modal-body p.identified-user-message').html(o.identifiedUserMessage),
          o.paypalPhone && $('.payment-paypal-help-number').text(o.paypalPhone),
          o.paypalImg
            ? $('.payment-paypal-title-short-logo').css('background-image', `url(${o.paypalImg})`)
        ...
        o.address1Placeholder &&
          $('.vtex-omnishipping-1-x-address input#ship-street').attr('placeholder', o.address1Placeholder),
          o.address2Placeholder &&
            $('.vtex-omnishipping-1-x-address input#ship-complement').attr('placeholder', o.address2Placeholder)
      }
      paymentBuilder(e) {
        let o = this
      ...
      customAddressFormLoader() {
        ...
        if (!vtex.googleMapsApiKey)
          return console.log('You might need to add your Google Maps API Key in your admin'), !1
  ...
  function (e, o, a) {
    const { _countries: n, _cities: r, _addressPlaceholder: t, _countriesrules: s } = a(7),
      { _locale: d } = a(0)
    e.exports = class {
      ...
      setForm(e = '', o = '', a = '', n = '', r = '', t = '', s = '', d = '', m = '', h = '') {
        $('.vcustom--vtex-omnishipping-1-x-address #v-custom-ship-street').val(this.addressrules.number ? o : a),
          ...
          $('.vcustom--vtex-omnishipping-1-x-address #v-custom-ship-street').attr('data-street', 'USA' == e ? a : o),
      ...
      updateGoogleForm(e = 'us') {
        $('input#v-custom-ship-street').attr('placeholder', t[e.toUpperCase()] ? t[e.toUpperCase()] : ''),
          ...
          e.gPlacesAutocomplete.addListener('place_changed', function () {
            let o = e.gPlacesAutocomplete.getPlace(),
              ...
              d = e.addressrules.postalCode
                ? e.returnAddressFRules(o.address_components, 'postal_code', 'long_name')
                : '00000'
            e.addressrules.number &&
              $('.vcustom--vtex-omnishipping-1-x-address #ship-number').val(
                e.returnAddressFRules(o.address_components, 'street_number', 'long_name')
              )
            ...
            let l = $('.street-address', i).text(),
              u =
                $('.locality', i).text() ||
                o.address_components.filter((e) => 'administrative_area_level_2' == e.types[0])[0].long_name
            e.setForm(a, r, l, m, d, u, t, h, s, C),
              e.validateAllFields(),
              e.updateAddress(a, d, u, t, r, m, h, o.formatted_address, e.address.addressId, C)
          }),
            ...
            .then(function (h) {
              h.error
                ...
                : vtexjs.checkout.getOrderForm().done(function (h) {
                    i.updateAddress(e, r, t, n, o, a, s, '', d || '', m || '', C || []),
      ...
      getCountries() {
        ...
        return e.deliveryCountries.map((o) => {
          ...
          if (a.countries[o])
            return `<option value="${o}" ${o == e.mainCountry ? 'selected' : ''}>${a.countries[o]}</option>`
        })
      }
      ...
      form(e) {
        let o = this,
          ...
          t = `\n      <div class="vcustom--vtex-omnishipping-1-x-address step">\n        <div>\n        <form>\n            <p class="input v-custom-ship-street required text"><label for="v-custom-ship-street">${
            o.locale.address1Placeholder ? o.locale.address1Placeholder : 'Street address or P.O. Box'
            ...
            o.locale.address2Placeholder ? o.locale.address2Placeholder : 'Apartment number, unit, floor, etc.'
        ...
        a.address && $('.vcustom--vtex-omnishipping-1-x-address #ship-state').val(a.address.state),
      ...
      validateAllFields() {
        let e = this
        ;(e.validate = !0),
          $('.vcustom--vtex-omnishipping-1-x-address input:required').each(function (o) {
      ...
      updateFormByCountry(e) {
        ...
        ;(this.addressrules = this.getCountryRule(o[1])),
          ...
          $("select[name='v-custom-state']").html('' + this.getRegions(o[0]).join('')),
      ...
      bind() {
        ...
        $('body').on(
          ...
          function (o) {
            ...
            if (
              ((n = a < 0 ? n.selectedAddresses[0] : n.availableAddresses[a]), n && n.street && !~n.street.indexOf('*'))
            )
              try {
                ;(n.isDisposable || ~window.location.origin.indexOf('myvtex')) &&
                  setTimeout(() => {
                    if (!$('.vtex-omnishipping-1-x-address').length) {
                      $('body').addClass(e.BodyFormClasses.join(' ')),
                        ...
                        $('.vcustom--vtex-omnishipping-1-x-address #ship-country').val(n.country),
                        e.updateFormByCountry(n.country),
                        e.getCountryRule(n.country).number &&
                          $('.vcustom--vtex-omnishipping-1-x-address #ship-number').val(n.number),
                        $('.vcustom--vtex-omnishipping-1-x-address #v-custom-ship-street')
                          .val(n.street)
                          .attr('data-street', ''),
                        $('.vcustom--vtex-omnishipping-1-x-address #ship-state').val(n.state),
                        $('.vcustom--vtex-omnishipping-1-x-address #ship-city').val(n.city)
                    }
                  }, 100)
              } catch (o) {
          ...
          $('body').on('click', '.vtex-omnishipping-1-x-addressItemOption', function (o) {
            e.address.addressId = e.orderForm.shippingData.availableAddresses[$(this).index()]
              ? e.orderForm.shippingData.availableAddresses[$(this).index()].addressId
              : ''
          }),
...
function shippingDay() {
  ...
  if (
    $('.vtex-omnishipping-1-x-leanShippingTextLabelSingle span').length &&
    !$('.vtex-omnishipping-1-x-leanShippingTextLabelSingle span').hasClass('renew')
  ) {
  ...
  if (
    $('ul.cart-items li.hproduct.item .description span').length &&
    !$('ul.cart-items li.hproduct.item .description span').hasClass('renew')
  ) {
...
const validateInputsProfile = () => {
  if (window.location.hash === '#/profile') {
    ...
    if (goToShippingBtn.style.display === 'none') {
      // Está activo el botón de ir al pago
  ...
  if (window.location.hash === '#/shipping') {
    ...
    if (goToPaymentBtn) {
      goToPaymentBtn.addEventListener('click', (e) => {
        // Si algún input tiene error
        if (
          shipNumber.classList.contains('error') ||
          shipComplement.classList.contains('error') ||
          shipReceiverName.classList.contains('error')
        ) {
          e.stopPropagation()
